...
**一条判定如果点名了这个任务拿不到的工具，加载任务时就会报错。** 只读任务上的
`not-used-tool: write_file` 永远不可能失败——它断言模型没调用一个它从没拿到过的工具；
反过来 `used-tool` 一个拿不到的工具则永远不可能通过。两者都是任务定义写错了，都在花第一
分钱之前拦下来。这类判定曾经在仓库里躺了两个版本。

有三条判定的存在，是因为"用过哪些工具"这个集合回答不了它们回答的问题。
`tool-never-failed` 抓瞎猜路径——猜错的读必然报错，而去重后的名字集合看不见这件事。
它只在"失败即出错"的工具上成立：`read_file` 成立，`bash` 不成立——后者对任何非零退出码
都报错，而在"先跑一次让它挂"的任务上，失败正是对的。`max-tool-calls` 卡的是调用次数
而不是轮数，因为一轮里可以发起任意多次调用："到底是顺着 import 找过去，还是把整个项目
读了一遍"，这是个次数问题；
`file-unchanged` 拿 fixture 原件做字节比较，把"别动没让你动的东西"从愿望变成判定，
而且不需要在 `task.json` 里再抄一份文件内容（抄一份就会有忘记同步的那天）。
JSON 报告里多了 `toolCalls`：按调用顺序记录每一次调用和它成功与否，和表格用的
`toolsUsed` 集合并存。

九个任务分成"找到了什么"和"改了什么"两类。其中四个的存在理由是：第一次真实基线
30 次里过了 29 次，也就是什么都没测出来——`needle-in-many-files`（40 个文件名毫无
信息量，`list_files` 给不出线索）、`trace-the-default`（一个值被覆盖两次，停在第一跳
会得到一个看起来很合理的错数字）、`decoy-config`（两个几乎一样的模块，其中一个是死代码）、
`wrong-test`（挂掉的测试，其期望与源码里写明的契约矛盾，改源码去迎合它就是陷阱）。

**一个任务如果在不同模型上都是 0/N 或都是 N/N，它就没在测东西**，应该改难、改对，
或者删掉。全都过不是质量的证据。
...
`--tui` 会在提示行上方画一块状态区域：第几轮、模型在想还是在跑工具、哪几个工具在跑、
各自针对什么、最新一行输出、已经跑了多久、累计花了多少 token。
...
它是事件流的第一个**反向**消费者。其他所有消费者——流式输出、会话日志、扩展、远程
客户端——都是追加式的：事件来了就打印，打印完就忘掉。这一个必须从同一条流里**重建出
"现在是什么状态"**，并随着新事件到达保持它正确。这件事拆成三段：

```text
reduce(state, event, now)  ->  ViewState     纯函数
view(state, {width, now})  ->  string[]      纯函数
paint(previous, next)                        唯一碰终端的地方
```

两个纯函数不需要终端就能测，真正的保证在那里。让状态模型成为承重结构的，是事件词汇表
里的一处不对称：`message_update` 带的是**增量**，`tool_execution_update` 带的是被截断过
的**快照**。对快照做追加，会堆出从未产生过的输出——那个窗口的头部早就被丢掉了；对增量
做替换，则只剩最后一个 token，整个回答就没了。追加式渲染器**永远不需要知道这个区别**，
因为它只管把收到的东西打出去。有两条测试，写反哪一边红哪一边。

它**不接管备用屏**。切过去滚动历史就没了，要补回来就得重新实现滚动、搜索和选中——
几千行，而且教的是终端图形，不是 Agent 架构。已完成的一轮会被刷进终端自己的滚动历史
并从状态里丢掉，所以再长的会话也不会把状态撑大。输入继续交给 readline 也是同一个理由：
行编辑器不是这里值得解决的问题。

绘制只重写内容变了的行，所以一个跳动的计时器只花一行，而不是重画整块。改变窗口大小会
让这个基准失效——终端已经把屏幕上的内容重新折行了——那是唯一一个"整块重画才是对的"
时刻。
...
npm run demo:tui  # 离线交互演示，无需 API Key 或网络
```

演示使用模拟供应商与工具进度，不修改工作区。操作说明和验证方式见 [TUI 指南](docs/tui.md)。
