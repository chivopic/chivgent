...
**T3 — `decoy-config`'s `not-used-tool: write_file` is stricter than the
prompt.** A model that correctly identifies `src/settings/http.ts` and rewrites
