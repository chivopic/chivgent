...
> 目标版本：`0.10.0`
> 状态：**设计中，尚未实现**（前七份文档都是实现之后补写的，这一份先于实现）
...
错误、命令的退出信息都在末尾；读文件才是要开头。这个差异值得单独说，因为
`truncate` 模块会被两边共用，必须同时提供 `truncateHead` 和 `truncateTail`。
...
4. 取消：abort 之后进程组被杀，工具在合理时间内返回，结果标记为中断。
...
第 4 项要验证的是"孙进程也死了"，用 `sh -c 'sleep 30 & wait'` 之类的结构，检查
组内进程确实消失——只检查工具返回是测不出这个 bug 的。

## 11. 验收标准

- [ ] `--allow-shell` 关闭时 `bash` 工具不存在；开启时命令可执行。
- [ ] 取消一次运行会杀掉整棵进程树，不留孤儿进程。
- [ ] chivgent 自身收到 SIGINT/SIGTERM 时清理所有 detached 子进程。
- [ ] 输出边跑边通过 `tool_execution_update` 呈现，且有节流。
- [ ] 超量输出被尾部截断，完整内容落在临时文件且路径可用。
- [ ] 内存占用不随输出长度线性增长。
- [ ] 非零退出码返回错误但保留输出。
- [ ] `npm run check`、`npm test`、`npm run build` 全部通过。
- [ ] README（中英）的安全模型改写为"粗粒度开关 + 容器化"，路线图同步。
- [ ] 版本提升到 `0.10.0`。
