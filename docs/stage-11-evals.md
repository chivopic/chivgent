...
> 目标版本：`0.13.0`
> 状态：**设计中，尚未实现**（与 Stage 8-10 一样先于实现写成）
...
- [ ] `npm run eval` 能跑完第一批任务并输出通过率表格。
- [ ] 每次尝试在独立的临时工作区中运行，互不影响。
- [ ] 失败带具体原因，不只是"未通过"。
- [ ] 工具使用作为判定条件生效（用错工具即使结果正确也算失败）。
- [ ] Runner 自身有单元测试，且不需要凭证、不花钱。
- [ ] evals 不进 CI 门禁，`npm test` 依然免费且确定。
- [ ] JSON 报告可用于跨版本对比。
- [ ] 缺凭证时报出清晰的配置错误。
- [ ] `npm run check`、`npm test`、`npm run build` 全部通过。
- [ ] README（中英）新增 evals 章节，说明它与测试的区别。
- [ ] 版本提升到 `0.13.0`。
