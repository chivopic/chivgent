...
## 11.1 `tool-never-failed` 只在"失败即出错"的工具上成立（Stage 13 加入）

`ok: false` 来自工具结果的 `isError`，而 `BashTool` 对**任何非零退出码**都置
`isError: true`（`src/tools/bash.ts`）。所以在 `fix-failing-test`、`wrong-test`
这类"先跑一次让它挂"的任务上，一次失败的 `bash` 调用是**正确行为**，
`tool-never-failed bash` 会把做对的模型判失败。

`read_file` 上两者重合——读一个不存在的路径既是失败也是出错——所以那里是成立的。
用这条判定前先问一句：这个工具"失败"是不是必然意味着模型错了？
