...
> 目标版本：`0.11.0`
> 状态：**设计中，尚未实现**（与 Stage 8 一样先于实现写成）
...
- [ ] 项目扩展在没有信任记录且非 TTY 时不被加载。
- [ ] 信任决定持久化，按最近祖先命中，可用 `--forget-trust` 撤销。
- [ ] 询问文案说明后果（进程内执行、不受工作区限制、不需要 `--allow-shell`）。
- [ ] 用户级扩展始终加载，不受项目信任影响。
- [ ] 四个 API 入口都生效。
- [ ] 任何单个扩展的失败都不影响启动和其他扩展。
- [ ] 扩展无法覆盖内置工具。
- [ ] `/help` 和 `/tools` 能区分内置与扩展来源。
- [ ] `npm run check`、`npm test`、`npm run build` 全部通过。
- [ ] README（中英）新增扩展章节，安全模型说明信任的含义。
- [ ] 版本提升到 `0.11.0`。
